...
// pub async fn orders_list_handler(db: DB) -> WebResult<impl Reply> {
//     let orders = repository::get_all_items(&db)
//         .await
//         .map_err(|e| reject::custom(e))?;
//     Ok(json(&orders))
// }

// pub async fn order_with_id_handler(id: String, db: DB) -> WebResult<impl Reply> {
//     let order = repository::get_item_with_id(&id, &db)
//         .await
//         .map_err(|e| reject::custom(e))?;
//     Ok(json(&order))
// }
...
pub async fn update_order_handler(id: String, body: Order, db: DB) -> WebResult<impl Reply> {
    repository::update_order_with_id(&id, &body, &db)
        .await
        .map_err(|e| reject::custom(e))?;
